...
//    MVCFramework.WebSocket.Server,
    ...
    GWSServer := TMVCWebSocketServer.Create(8091);
    GWSServer.OnError := procedure(AClient: TWebSocketClient; const AError: string)
    begin
      Writeln('WSS ERROR: ' + AError);
    end;
    GWSServer.Active := True;

    DefaultMVCServiceContainer.Build;

    RunServer(dotEnv.Env('dmvc.server.port', 8090));
